...
const commandOnOff = (req, res) => {
  ...
  if (req.decoded.level === 'administrator' || req.decoded.level === 'manager') {
    res.json({
      valid: false,
      messages: ['Anda tidak mempunyai izin akses untuk menghidupkan atau mematikan perangkat'],
    });
  }
      ...
      Device.findOne({
        ...
        if (err) {
          ...
          if (doc.online === 0) {
            res.json({
              valid: false,
              messages: ['Status perangkat dalam keadaan offline, mohon aktifkan perangkat telebih dahulu'],
            });
          }
          ...
          if (doc === null) {
            callback({
              valid: false,
              messages: ['Device ID belum terdaftar'],
            });
          } else if (doc.user.username === req.decoded.username) {
            ...
            res.json({
              valid: false,
              messages: ['Device ID bukan milik pengguna aktif'],
            });
          }
        }
      });
